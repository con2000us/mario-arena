 ...
 *
 * Physics units are px/frame at a fixed 60 Hz timestep. The speed tiers,
 * jump profiles, friction, and gravity below are translations of the public
 * SMB disassembly tables (1/16 horizontal speed and 1/256 force fractions).
 * ============================================================================ */
...
const WALK_MAX = 1.5, RUN_MAX = 2.5;
const ACCEL_WALK = 0x98 / 256 / 16;   // FrictionData[1]
const ACCEL_RUN = 0xe4 / 256 / 16;    // FrictionData[0]
const ACCEL_FAST = 0xd0 / 256 / 16;   // FrictionData[2]
const DECEL = ACCEL_WALK;
const SKID_DECEL = (0x98 * 2) / 256 / 16;
const AIR_TURN = (0xe4 * 2) / 256 / 16;
const JUMP_HOLD = [0x20, 0x20, 0x1e, 0x28, 0x28].map(function (n) { return n / 256; });
const JUMP_FALL = [0x70, 0x70, 0x60, 0x90, 0x90].map(function (n) { return n / 256; });
const JUMP_INITIAL = [-4, -4, -4, -5, -5];
const MAX_FALL = 4;
...
let gameState = 'title';           // title | intro | play | gameover
let paused = false;
let globalFrame = 0;
let introT = 0;
...
function clearInputState() {
  input.left = input.right = input.down = input.jump = input.run = false;
  input.jumpPressed = input.runPressed = false;
  if (typeof document !== 'undefined' && document.querySelectorAll) {
    document.querySelectorAll('[data-key].pressed').forEach(function (el) {
...
function resetLevel() {
  ...
  camX = 0; enemyEventIdx = 0; freezeT = 0;
  ...
  mario = {
    ...
    runT: 0, jumpProfile: 0, pipeTargetY: 0,
    shrinkStayBig: false,
    hitWall: 0, landed: false, hitHead: false, headTx: 0, headTy: 0
  };
  cameraMarioX = mario.x;
}
...
function spawnEnemyPx(kind, xPx, yPx) {
  if (kind === 'goomba') {
    ents.push({ kind: 'goomba', x: xPx + 1, y: yPx, w: 14, h: 14,
                vx: -0.5, vy: 0, dir: -1, state: 'walk', t: 0 });
  } else if (kind === 'koopa') {
    ents.push({ kind: 'koopa', x: xPx + 2, y: yPx, w: 12, h: 22,
                vx: -0.5, vy: 0, dir: -1, state: 'walk', t: 0, shellT: 0, kills: 0 });
  }
}

function spawnEnemy(kind, tileX, yPx) {
  spawnEnemyPx(kind, tileX * TILE, yPx);
}

function spawnEnemiesAhead() {
  if (area !== 'overworld') return;
  const events = LEVEL_1_1.enemyEvents;
  // The original parser checks the right screen edge extended by 48 pixels,
  // rounded to a metatile boundary. Group commands then instantiate enemies at
  // the live right edge, 24 pixels apart, instead of at fixed map markers.
  const extendedRight = Math.floor((camX + VW + 48) / TILE) * TILE;
  while (enemyEventIdx < events.length && events[enemyEventIdx].triggerX <= extendedRight) {
    const e = events[enemyEventIdx];
    if (e.group === 1) {
      spawnEnemyPx(e.kind, e.x, e.y);
    } else {
      const base = Math.floor(camX + VW);
      for (let n = 0; n < e.group; n++) spawnEnemyPx(e.kind, base + n * 24, e.y);
    }
    enemyEventIdx++;
  }
}
...
/* ---- pipe travel + bonus room ---------------------------------------------------- */

function startPipeDown() {
  const p = LEVEL_1_1.entryPipe;
  mario.state = 'pipeDown';
  mario.stateT = 0;
  mario.vx = 0; mario.vy = 0;
  mario.x = p.left * TILE + (2 * TILE - mario.w) / 2;
  mario.facing = 1;
  input.jumpPressed = false; input.runPressed = false;
  Music.stop();
  Sfx.pipe();
}
...
function enterBonus() {
  area = 'bonus';
  map = BONUS_1_1.rows.map(function (r) { return r.split(''); });
  LW = BONUS_1_1.width; LH = BONUS_1_1.height; levelWpx = LW * TILE;
  ents = []; bumps = []; popups = []; particles = [];
  camX = 0;
  mario.x = 32;
  mario.y = 32;
  mario.vx = 0; mario.vy = 0;
  mario.state = 'bonusDrop';
  mario.stateT = 0;
  mario.onGround = false;
  mario.visible = true;
  cameraMarioX = mario.x;
  Music.play(mario.starT > 0 ? 'star' : 'bonus');
}

function startBonusExit() {
  mario.state = 'pipeRight';
  mario.stateT = 0;
  mario.vx = 0; mario.vy = 0;
  mario.x = BONUS_1_1.exitPipeCol * TILE - mario.w;
  Music.stop();
  Sfx.pipe();
}

function returnOverworld() {
  area = 'overworld';
  map = overworldMap || LEVEL_1_1.rows.map(function (r) { return r.split(''); });
  LW = LEVEL_1_1.width; LH = LEVEL_1_1.height; levelWpx = LW * TILE;
  ents = []; bumps = []; popups = []; particles = [];
  const p = LEVEL_1_1.returnPipe;
  mario.x = p.left * TILE + (2 * TILE - mario.w) / 2;
  mario.y = 13 * TILE;
  mario.vx = 0; mario.vy = 0;
  mario.state = 'pipeUp';
  mario.stateT = 0;
  mario.pipeTargetY = p.topRow * TILE - mario.h;
  mario.onGround = false;
  mario.visible = true;
  camX = Math.max(0, Math.min(levelWpx - VW, mario.x - 112));
  cameraMarioX = mario.x;
  // Everything bypassed through the bonus room is consumed by the area parser.
  while (enemyEventIdx < LEVEL_1_1.enemyEvents.length &&
         LEVEL_1_1.enemyEvents[enemyEventIdx].triggerX < mario.x - 48) enemyEventIdx++;
  Music.play(mario.starT > 0 ? 'star' : 'main');
}

function updatePipeState() {
  mario.stateT++;
  if (mario.state === 'pipeDown') {
    mario.y += 1;
    if (mario.stateT >= 32) enterBonus();
  } else if (mario.state === 'bonusDrop') {
    mario.vy = Math.min(MAX_FALL, mario.vy + 0x70 / 256);
    moveEntityY(mario);
    if (mario.landed) {
      mario.onGround = true;
      mario.state = 'play';
  ...
  } else if (mario.state === 'pipeRight') {
    mario.x += 1;
    if (mario.stateT >= 32) returnOverworld();
  } else if (mario.state === 'pipeUp') {
    mario.y -= 1;
    if (mario.y <= mario.pipeTargetY) {
      mario.y = mario.pipeTargetY;
      mario.state = 'play';
      mario.stateT = 0;
      mario.onGround = true;
    }
...
function collectMapCoins() {
  const tx0 = Math.floor(mario.x / TILE), tx1 = Math.floor((mario.x + mario.w - 0.001) / TILE);
  const ty0 = Math.floor(mario.y / TILE), ty1 = Math.floor((mario.y + mario.h - 0.001) / TILE);
  for (let ty = ty0; ty <= ty1; ty++) {
    for (let tx = tx0; tx <= tx1; tx++) {
      if (tileAt(tx, ty) === 'O') {
        map[ty][tx] = '.';
        awardCoin(tx * TILE + 4, ty * TILE - 4);
        Sfx.coin();
      }
    }
  }
...
function updateMario() {
  ...
  if (mario.starT > 0) {
    ...
    if (mario.starT === 0) {
      ...
      Music.play(area === 'bonus' ? 'bonus' : 'main');
    }
  }
  ...
  switch (mario.state) {
    ...
    case 'bonusDrop':
    case 'pipeRight':
    case 'pipeUp': updatePipeState(); break;
...
function updateMarioPlay() {
  // Down on the fourth surface pipe enters the documented 1-1 bonus area.
  if (area === 'overworld' && input.down && mario.onGround) {
    const p = LEVEL_1_1.entryPipe;
    const center = mario.x + mario.w / 2;
    const feet = mario.y + mario.h;
    if (center >= p.left * TILE && center < (p.right + 1) * TILE &&
        Math.abs(feet - p.topRow * TILE) < 2) {
      startPipeDown();
      return;
    }
  }
  if (area === 'bonus' && input.right && mario.onGround &&
      mario.x + mario.w >= BONUS_1_1.exitPipeCol * TILE - 0.5) {
    startBonusExit();
    return;
  }

  // Crouch (big Mario only). Expansion is blocked under low ceilings.
  ...
  // SMB keeps a ten-frame running latch after B is released.
  if (input.run) mario.runT = 10;
  else if (mario.runT > 0) mario.runT--;
  const running = mario.runT > 0;
  const max = running ? RUN_MAX : WALK_MAX;
  const sameDirAccel = running ? (Math.abs(mario.vx) >= 0x21 / 16 ? ACCEL_FAST : ACCEL_RUN) : ACCEL_WALK;
  ...
  if (!mario.crouching) {
    if (input.left && !input.right) {
      mario.facing = -1;
      if (mario.vx > 0) {
        mario.vx -= mario.onGround ? SKID_DECEL : AIR_TURN;
        if (mario.onGround && mario.vx > 0x0b / 16) mario.skidding = true;
      } else if (-mario.vx > max) {
        mario.vx = Math.min(-max, mario.vx + DECEL);
      } else {
        mario.vx = Math.max(-max, mario.vx - sameDirAccel);
      }
      ...
      if (mario.vx < 0) {
        ...
        mario.vx = Math.max(max, mario.vx - DECEL);
      } else {
        mario.vx = Math.min(max, mario.vx + sameDirAccel);
      }
  ...
  // Five jump profiles selected by horizontal speed, matching the source tables.
  if (input.jumpPressed && mario.onGround && !mario.crouching) {
    mario.jumpProfile = jumpProfile(mario.vx);
    mario.vy = JUMP_INITIAL[mario.jumpProfile];
    mario.onGround = false;
    mario.jumpHeld = true;
    Sfx.jump(mario.big);
  }
  if (!input.jump) mario.jumpHeld = false;
  const jp = mario.jumpProfile;
  if (mario.vy < 0 && mario.jumpHeld && input.jump) mario.vy += JUMP_HOLD[jp];
  else mario.vy += JUMP_FALL[jp];
  if (mario.vy > MAX_FALL) mario.vy = MAX_FALL;

  // Fire Mario may have at most two live fireballs.
  if (input.runPressed && mario.fire && !mario.crouching && mario.throwT <= 0) {
    ...
    if (n < 2) {
      ents.push({ kind: 'fireball',
                  ...
                  y: mario.y + 4, w: 8, h: 8,
                  vx: 4.0 * mario.facing, vy: 0.6, t: 0 });
      mario.throwT = 12;
      Sfx.fireball();
    }
  }
  ...
  // The camera never permits backtracking beyond its left edge.
  ...
  // Re-check the bonus exit after horizontal collision has placed Mario flush
  ...
  if (area === 'overworld' && mario.x + mario.w >= poleX && mario.x < poleX) startPoleSequence();
}
...
function startPoleSequence() {
  const grabY = mario.y;
  // FlagpoleYPosData ($18,$22,$50,$68,$90) is scanned from index 4 down to 1;
  // index 0 is the fallback for Y < $22. Scores are 5000,2000,800,400,100.
  const tier = grabY < 34 ? 5000 : grabY < 80 ? 2000 : grabY < 104 ? 800 :
               grabY < 144 ? 400 : 100;
...
function updateCamera() {
  const dx = mario.x - cameraMarioX;
  cameraMarioX = mario.x;
  if (mario.state !== 'play' || dx <= 0) return;

  // Source behavior: scrolling starts when Mario reaches screen x=80. At
  // one-pixel walking speed the camera follows fully; at two-or-more-pixel run
  // speed it lags by one pixel until Mario reaches x=112, then follows fully.
  let screenX = mario.x - camX;
  if (screenX >= 80) {
    let amount = dx;
    if (screenX < 112 && dx >= 2) amount = dx - 1;
    camX += Math.max(0, amount);
    screenX = mario.x - camX;
    if (screenX > 112) camX += screenX - 112;
  }
  camX = Math.max(0, Math.min(camX, Math.max(0, levelWpx - VW)));
}
...
function drawDecor(cam) {
  ...
  for (const d of LEVEL_1_1.decor) {
    let x = Math.floor(d.x * TILE - cam);
    if (x < -96 || x > VW + 32) continue;

    if (d.t === 'hill') {
      const w = d.tiles * TILE - 2;
      const h = d.tiles === 5 ? 34 : 18;
      x += 1;
      // Two-pixel stair steps avoid antialiased vector edges.
      for (let yy = 0; yy < h; yy += 2) {
        const half = Math.max(1, Math.floor((yy + 2) * w / h / 2));
        ctx.fillStyle = '#005800';
        ctx.fillRect(Math.floor(x + w / 2 - half), groundY - h + yy, half * 2, 2);
      }
      for (let yy = 2; yy < h; yy += 2) {
        const half = Math.max(1, Math.floor(yy * (w - 4) / h / 2));
        ctx.fillStyle = '#00A800';
        ctx.fillRect(Math.floor(x + w / 2 - half), groundY - h + yy, half * 2, 2);
      }
      ctx.fillStyle = '#80D010';
      ctx.fillRect(Math.floor(x + w / 2 - 2), groundY - h + 6, 3, 2);
      ctx.fillRect(x + Math.floor(w * .28), groundY - 10, 2, 3);
      ctx.fillStyle = '#005800';
      ctx.fillRect(x + Math.floor(w * .65), groundY - 16, 2, 3);
    } else if (d.t === 'bush') {
      const w = d.tiles * TILE - 2;
      x += 1;
      ctx.fillStyle = '#005800';
      ctx.fillRect(x, groundY - 8, w, 8);
      for (let bx = 0; bx < d.tiles; bx++) {
        const peak = (bx === Math.floor(d.tiles / 2) && d.tiles > 2) ? 15 : 12;
        ctx.fillRect(x + bx * 16 + 2, groundY - peak, 12, peak - 7);
        ctx.fillRect(x + bx * 16, groundY - peak + 3, 16, peak - 6);
      }
      ctx.fillStyle = '#00A800';
      ctx.fillRect(x + 2, groundY - 7, w - 4, 7);
      for (let bx = 0; bx < d.tiles; bx++) {
        const peak = (bx === Math.floor(d.tiles / 2) && d.tiles > 2) ? 14 : 11;
        ctx.fillRect(x + bx * 16 + 4, groundY - peak, 8, peak - 6);
        ctx.fillRect(x + bx * 16 + 2, groundY - peak + 3, 12, peak - 5);
      }
      ctx.fillStyle = '#80D010';
      ctx.fillRect(x + 7, groundY - 9, 2, 2);
      if (w > 40) ctx.fillRect(x + w - 17, groundY - 7, 2, 2);
    } else if (d.t === 'cloud') {
      const y = d.y * TILE + 1;
      const w = d.tiles * TILE - 2;
      x += 1;
      ctx.fillStyle = '#3CBCFC';
      ctx.fillRect(x + 2, y + 9, w - 4, 12);
      for (let bx = 0; bx < d.tiles; bx++) {
        const tall = d.tiles > 2 && bx === Math.floor((d.tiles - 1) / 2);
        ctx.fillRect(x + bx * 16 + 2, y + (tall ? 2 : 6), 12, tall ? 17 : 13);
        ctx.fillRect(x + bx * 16, y + (tall ? 6 : 9), 16, tall ? 13 : 10);
      }
      ctx.fillStyle = '#FCFCFC';
      ctx.fillRect(x + 3, y + 9, w - 6, 9);
      for (let bx = 0; bx < d.tiles; bx++) {
        const tall = d.tiles > 2 && bx === Math.floor((d.tiles - 1) / 2);
        ctx.fillRect(x + bx * 16 + 4, y + (tall ? 3 : 7), 8, tall ? 13 : 9);
        ctx.fillRect(x + bx * 16 + 2, y + (tall ? 7 : 10), 12, tall ? 10 : 6);
      }
      ctx.fillStyle = '#3CBCFC';
      ctx.fillRect(x + 5, y + 16, 3, 2);
      ctx.fillRect(x + w - 10, y + 14, 3, 2);
    }
  }
}
...
function drawBonusPipes(cam) {
  const vx = 15 * TILE - cam, vy = 2 * TILE;
  // Vertical return pipe rising beyond the top of the visible chamber.
  ctx.fillStyle = '#005800'; ctx.fillRect(vx, vy, 32, 176);
  ctx.fillStyle = '#80D010'; ctx.fillRect(vx + 3, vy, 5, 176);
  ctx.fillStyle = '#00A800'; ctx.fillRect(vx + 8, vy, 16, 176);
  ctx.fillStyle = '#005800'; ctx.fillRect(vx + 24, vy, 7, 176);
  ctx.fillStyle = '#000'; ctx.fillRect(vx + 31, vy, 1, 176);

  // Two-metatile horizontal mouth/body connected at floor level.
  const hx = 13 * TILE - cam, hy = 11 * TILE;
  ctx.fillStyle = '#005800'; ctx.fillRect(hx, hy, 64, 32);
  ctx.fillStyle = '#80D010'; ctx.fillRect(hx + 3, hy + 3, 4, 26);
  ctx.fillStyle = '#00A800'; ctx.fillRect(hx + 7, hy + 3, 45, 26);
  ctx.fillStyle = '#005800'; ctx.fillRect(hx + 52, hy + 3, 12, 26);
  ctx.fillStyle = '#000'; ctx.fillRect(hx, hy, 2, 32);
  ctx.fillRect(hx, hy, 16, 3); ctx.fillRect(hx, hy + 29, 16, 3);
  ctx.fillStyle = '#9CFCF0'; ctx.fillRect(hx + 4, hy + 5, 2, 20);
}
...
function drawTiles(cam) {
  ...
  for (let ty = 0; ty < LH; ty++) {
    for (let tx = tx0; tx <= tx1; tx++) {
      ...
      if (area === 'bonus') {
        if (c === 'P') continue;                // one connected pipe overlay
        if (c === 'O') {
          drawSprite(ctx, SPRITES.coin, Math.floor(globalFrame / 4) % 4,
                     tx * TILE - cam + 4, ty * TILE);
          continue;
        }
        if (c === 'X') {
          drawUndergroundBlock(tx * TILE - cam, ty * TILE);
          continue;
        }
      }
      let img = null;
...
function renderTitle() {
  ...
  drawText(ctx, 'SUPER MARIO BROS.', centerX('SUPER MARIO BROS.', 2) + 2, 39, '#7C0800', 2);
  drawText(ctx, 'SUPER MARIO BROS.', centerX('SUPER MARIO BROS.', 2), 36, '#F8B800', 2);
  drawText(ctx, 'WORLD 1-1 EDITION', centerX('WORLD 1-1 EDITION', 1), 64, '#FCFCFC', 1);
  ...
  drawText(ctx, '(C) 2026 CLEAN-ROOM BUILD', centerX('(C) 2026 CLEAN-ROOM BUILD', 1), 184, '#FCFCFC', 1);
  drawText(ctx, 'NOT AFFILIATED WITH NINTENDO', centerX('NOT AFFILIATED WITH NINTENDO', 1), 196, '#FCFCFC', 1);
}

function renderIntro() {
  ctx.fillStyle = '#000';
  ctx.fillRect(0, 0, VW, VH);
  drawText(ctx, 'WORLD 1-1', centerX('WORLD 1-1', 1), 72, '#FCFCFC');
  drawSprite(ctx, SPRITES.marioSmall, 0, 91, 112);
  drawText(ctx, '*  ' + lives, 120, 118, '#FCFCFC');
}
...
function render() {
  ctx.fillStyle = area === 'bonus' ? '#000000' : SKY;
  ctx.fillRect(0, 0, VW, VH);
  ...
  drawTiles(cam);
  if (area === 'bonus') drawBonusPipes(cam);
...
function resizeCanvasInteger() {
  if (!canvas || typeof window === 'undefined') return;
  const coarse = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
  const sideControls = coarse && window.innerWidth > window.innerHeight && window.innerHeight <= 560;
  const availW = Math.max(256, window.innerWidth - (sideControls ? 190 : 16));
  const availH = Math.max(240, window.innerHeight - (coarse && !sideControls ? 150 : 40));
  const scale = Math.max(1, Math.min(3, Math.floor(Math.min(availW / VW, availH / VH))));
...
function setupPointerControls() {
  ...
  canvas.addEventListener('pointerdown', function (e) {
    ...
    if (gameState === 'title' || gameState === 'gameover' ||
        (gameState === 'play' && mario.state === 'clear')) handleEnter();
  });
...
function init() {
  ...
  if (typeof window !== 'undefined' && window.addEventListener) {
    window.addEventListener('keydown', function (e) {
      ...
      if (e.repeat && (e.code === 'Enter' || e.code === 'KeyM')) return;
      keyDown(e.code);
    });
    ...
    window.addEventListener('resize', resizeCanvasInteger);
    window.addEventListener('orientationchange', resizeCanvasInteger);
    window.addEventListener('blur', clearInputState);
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) {
        clearInputState();
        if (gameState === 'play' && !paused && mario.state !== 'die') {
          paused = true;
          Music.suspend();
        }
      }
...
const Game = {
  ...
  newGame: newGame,
  getInfo: function () {
    return { gameState: gameState, area: area, mario: mario, score: score, coins: coins,
             lives: lives, hiscore: hiscore, time: time, camX: camX, ents: ents.length,
             entityKinds: ents.map(function (e) { return e.kind; }),
             entities: ents.map(function (e) {
               return { kind: e.kind, state: e.state || '', x: e.x, y: e.y,
                        vx: e.vx || 0, vy: e.vy || 0, t: e.t || 0 };
             }),
             inputState: Object.assign({}, input), enemyEventIdx: enemyEventIdx,
             mapWidth: LW, mapHeight: LH, paused: paused };
  },
  getSpec: function () {
    return {
      viewport: [VW, VH], fixedHz: 60, levelColumns: LEVEL_1_1.width,
      physics: { walkMax: WALK_MAX, runMax: RUN_MAX, jumpInitial: JUMP_INITIAL.slice(),
                 jumpHold: JUMP_HOLD.slice(), jumpFall: JUMP_FALL.slice(), maxFall: MAX_FALL },
      enemyCommands: LEVEL_1_1.enemyEvents.map(function (e) { return Object.assign({}, e); }),
      enemyTotal: LEVEL_1_1.enemyEvents.reduce(function (n, e) { return n + e.group; }, 0),
      bonusCoins: BONUS_1_1.coinCount,
      entryPipe: Object.assign({}, LEVEL_1_1.entryPipe),
      returnPipe: Object.assign({}, LEVEL_1_1.returnPipe)
    };
  },
  debug: {
    skipIntro: function () { gameState = 'play'; introT = 150; Music.play('main'); },
    ...
    enterBonus: function () { enterBonus(); },
    leaveBonus: function () { returnOverworld(); },
    setMario: function (x, y, vx, vy, onGround) {
      mario.x = x; mario.y = y; mario.vx = vx || 0; mario.vy = vy || 0;
      mario.onGround = !!onGround; cameraMarioX = mario.x;
    },
    getTile: function (tx, ty) { return tileAt(tx, ty); },
    setTime: function (v) { time = v; timeFrames = 0; },
    startPole: function () { mario.x = poleX - mario.w; startPoleSequence(); }
  }
};
